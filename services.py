...
class ConversationalRAG:
    ...
    def _init_database(self):
        """初始化 SQLite 資料庫並創建 history 表格 (如果不存在)"""
        try:
    ...
    def add_history_entry(self, title: str, report: str) -> int:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO history (title, report) VALUES (?, ?)", (title, report))
    ...
    def get_history_list(self) -> list:
        """獲取所有歷史紀錄的列表 (不含報告全文)"""
        conn = sqlite3.connect(self.db_path)
        conn.row_factory = sqlite3.Row # 讓結果可以像字典一樣取值
        cursor = conn.cursor()
        cursor.execute("SELECT id, title, timestamp FROM history ORDER BY timestamp DESC")
    ...
    # --- (此處省略了 _init_playwright, _load_all_prompts, _init_prompts, set_llm_model, set_web_search 等函式, 它們保持不變) ---
